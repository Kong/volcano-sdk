...
Define specialized agents that autonomously coordinate based on their descriptions. Like automatic tool selection (`mcps`), but for agents.

### How It Works

When you provide `agents: [...]` in a step, the LLM automatically selects which agents to use and coordinates them to accomplish the task.
...
const researcher = agent({
  llm,
  name: "researcher",
  description: "Analyzes topics and provides factual information",
});
...
const editor = agent({
  llm,
  name: "editor",
  description: "Reviews and improves content for clarity",
});

// LLM autonomously coordinates them
await agent({ llm })
  .then({
...
// The LLM decides:
// 1. "I need facts" → USE researcher: Research AI
// 2. "Now create content" → USE writer: Write using [research]
// 3. "DONE: Final blog post..."
```

### Requirements

- Each agent must have `name` (unique identifier)
- Each agent must have `description` (what it does and when to use it)
- Good descriptions help the LLM make better choices

**💡 Pro Tip:** Write descriptions that explain WHEN to use each agent, not just WHAT it does. Example: "Use for complex analysis requiring data validation" is better than "Analyzes data".
...
await agent({ llm })
  .then({
    ...
    maxAgentIterations: 10, // Default: 4
  })
  .run();
```

### When to Use Agent Crews vs Manual Workflows
...
- When optimal sequence is unclear

**Use Manual Workflows for:**

- Fixed, predictable sequences
- Performance-critical paths
- When you know the exact steps
- Simpler, more deterministic tasks

### Observability

Agent crews are fully observable:

- **Spans:** step.type = `agent_crew`
- **Metrics:** `volcano.agent.calls.total` (by agent name)
- **Metrics:** `volcano.agent.delegation.count` (agents per step)
- **Metadata:** `agentCalls` in StepResult shows which agents were used
