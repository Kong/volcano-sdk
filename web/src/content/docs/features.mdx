...
Enable gorgeous TTY progress for any workflow:

```typescript
agent({ llm, showProgress: true })
  .then({ prompt: "Analyze data" })
...
```typescript
agent({ llm, showProgress: true })  // Beautiful default progress
  .then({
    agents: [researcher, writer],
...
**💡 Tip:** If you provide custom `onAgentEvent` or `onToken`, the default progress for that callback is disabled. To keep defaults, just use `showProgress: true` without custom callbacks.
