...
Let specialized agents autonomously coordinate to accomplish complex tasks. Similar to automatic tool selection (`mcps`), but for agents.
...
// LLM decides which agents to use and when:
...
When you provide `agents: [...]`, the SDK automatically:

1. Injects agent descriptions into the prompt
2. LLM decides which agent to delegate to
3. Executes the selected agent
4. Feeds result back to coordinator
5. Repeats until LLM says `DONE: [final answer]`

### Requirements

- Each agent must have `name` (unique identifier)
- Each agent must have `description` (what it does and when to use it)
- The coordinator LLM uses descriptions to select appropriate agents

**💡 Pro Tip:** Write descriptions that explain WHEN to use each agent, not just WHAT it does.
...
Agent crews are fully observable via OpenTelemetry:
...
**Full details:** [Observability documentation](/docs/observability)
