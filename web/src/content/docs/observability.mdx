...
| **step.execute** | `step.index`, `step.type` (llm \| mcp_auto \| mcp_explicit \| agent_crew) |
...
| `volcano.agent.calls.total`       | Counter   | Total sub-agent delegations, labeled by agent name        |
| `volcano.agent.delegation.count`  | Histogram | Number of agents delegated to per coordination step       |
