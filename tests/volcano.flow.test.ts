    ...
    describe(`Provider: ${p.name}`, () => {
      it('runs a two-step automatic flow using both MCP servers', async () => {
        ...
        const results = await agent({ llm })
          .then({ prompt: 'Determine the astrological sign for the birthdate 1993-07-11 using available tools.', mcps: [astro] })
          .then({ prompt: 'Based on the astrological sign Cancer, determine my favorite food and drink using available tools.', mcps: [favorites] })
          .run();
